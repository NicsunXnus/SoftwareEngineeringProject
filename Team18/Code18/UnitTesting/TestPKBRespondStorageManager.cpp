 ...
 namespace UnitTesting {
 	TEST_CLASS(TestPKBRespondStorageManager) {
 		private:
 			...
 			vector<shared_ptr<Token>> tokens1 = SimpleProcessor::tokenizeArithmeticExpression("a * b / 3 % 5");
 			shared_ptr<Node> node_ptr1 = TreeBuilder(17).buildTree(tokens1, false);
 			vector<shared_ptr<Token>> tokens2 = SimpleProcessor::tokenizeArithmeticExpression("((a + (b * 3))) + (((5)))");
 			shared_ptr<Node> node_ptr2 = TreeBuilder(21).buildTree(tokens2, false);
 			vector<shared_ptr<Token>> tokens3 = SimpleProcessor::tokenizeArithmeticExpression("((a + b) * 3 + 5)");
 			shared_ptr<Node> node_ptr3 = TreeBuilder(6).buildTree(tokens3, false);
 			vector<shared_ptr<Token>> tokens4 = SimpleProcessor::tokenizeConditionalExpression("a > 1 || b < 2 || c == 3");
 			shared_ptr<Node> node_ptr4 = TreeBuilder(33).buildTree(tokens4, true);
 			map<string, shared_ptr<Node>> patternData = { {"17", node_ptr1 }, { "21", node_ptr2 }, {"6", node_ptr3 }, {"33", node_ptr4 } };
 			shared_ptr<map<string, shared_ptr<Node>>> patternMap = make_shared<map<string, shared_ptr<Node>>>(patternData);
 		...
 		public:
 			...
 			TEST_METHOD(TestGetPatterns) {
 				...
 				Assert::IsTrue((PKB::responder.getPattern("17")->isIdentical(node_ptr1, true)));
 				Assert::IsTrue((PKB::responder.getPattern("21")->isIdentical(node_ptr2, true)));
 				Assert::IsTrue((PKB::responder.getPattern("6")->isIdentical(node_ptr3, true)));
 				Assert::IsTrue((PKB::responder.getPattern("33")->isIdentical(node_ptr4, true)));
 			}
