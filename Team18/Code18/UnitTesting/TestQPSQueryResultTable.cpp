...

{
	...
	public:
		...
		/*TEST_METHOD(TestMajorTableOperationsLeftToRight) {
            ...
            vector<unordered_map<string, vector<string>>> columnsUses = { map1,map2 };
            shared_ptr<QueryResultsTable> tab1 = make_shared<QueryResultsTable>(columnsUses);

            unordered_map<string, vector<string>> map3;
            map3.insert({ "s3", {"6", "5","7"} });

            vector<unordered_map<string, vector<string>>> columnsModEnt = { map3 };
            shared_ptr<QueryResultsTable> tab2 = make_shared<QueryResultsTable>(columnsModEnt);

            unordered_map<string, vector<string>> map4;
            map4.insert({ "s1", {"6", "3", "10", "8"} });

            unordered_map<string, vector<string>> map5;
            map5.insert({ "s2", {"20", "4", "15", "11"} });

            vector<unordered_map<string, vector<string>>> columnsFollows = { map4, map5 };
            shared_ptr<QueryResultsTable> tab3 = make_shared<QueryResultsTable>(columnsFollows);


            unordered_map<string, vector<string>> map6;
            map6.insert({ "s3", {"5", "5", "2", "2", "7"} });

            unordered_map<string, vector<string>> map7;
            map7.insert({ "s1", {"6", "8", "7", "3", "6"} });

            vector<unordered_map<string, vector<string>>> columnsParent = { map6, map7 };
            shared_ptr<QueryResultsTable> tab4 = make_shared<QueryResultsTable>(columnsParent);

            unordered_map<string, vector<string>> map8;
            map8.insert({ "s2", {"11", "20", "4", "20", "11", "10"} });

            unordered_map<string, vector<string>> map9;
            map9.insert({ "v1", {"y", "x", "x", "y", "z", "z"} });


            vector<unordered_map<string, vector<string>>> columnsUses2 = { map8,map9 };
        ...
        TEST_METHOD(TestFilterCol1Col2) {
            ...
            vector<unordered_map<string, vector<string>>> columnsUses = { map1,map2,map3, map4 };
            shared_ptr<QueryResultsTable> tab1 = make_shared<QueryResultsTable>(columnsUses);
