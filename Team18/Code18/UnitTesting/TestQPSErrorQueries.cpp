...

{
	...
	public:
		...
		{
			vector<string> tokenizer = PQLTokenizer::tokenize("assign a; variable v; constant c; Select a such that Uses (a, c) pattern a (\"8\", _)");
		...
		{
			vector<string> tokenizer = PQLTokenizer::tokenize("stmt s, s1; Select s1 such that Follows (\"s\", s1)");
			vector<string_view> testSv{ sToSvVector(tokenizer) };
			shared_ptr<QueryParser> p = make_shared<QueryParser>();
		...
		{
			vector<string> tokenizer = PQLTokenizer::tokenize("Select s");
			vector<string_view> testSv{ sToSvVector(tokenizer) };
			shared_ptr<QueryParser> p = make_shared<QueryParser>();
