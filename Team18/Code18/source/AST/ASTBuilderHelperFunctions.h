...
// sources:
// https://learn.microsoft.com/en-us/office/vba/language/reference/user-interface-help/operator-precedence
//,
...
static std::shared_ptr<ExprNode> applyOperation(std::string op,
                                                std::shared_ptr<ExprNode> a,
                                                std::shared_ptr<ExprNode> b) {
  if (op == "+")
    return std::make_shared<PlusNode>(a, b);
  else if (op == "*")
    return std::make_shared<TimesNode>(a, b);
  else if (op == "-")
    return std::make_shared<MinusNode>(a, b);
  else if (op == "/")
    return std::make_shared<DividesNode>(a, b);
  else if (op == "%")
    return std::make_shared<ModNode>(a, b);
  else
    throw std::invalid_argument("Unsupported mathematical operation: " + op);
}

static shared_ptr<CondExprNode> applyCompOperation(string op,
                                                   shared_ptr<ExprNode> a,
                                                   shared_ptr<ExprNode> b) {
  if (op == "<")
    return make_shared<LesserThanNode>(a, b);
  else if (op == ">")
    return make_shared<GreaterThanNode>(a, b);
  else if (op == "<=")
    return make_shared<LesserThanEqualNode>(a, b);
  else if (op == ">=")
    return make_shared<GreaterThanEqualNode>(a, b);
  else if (op == "==")
    return make_shared<EqualsNode>(a, b);
  else if (op == "!=")
    return make_shared<NotEqualsNode>(a, b);
  else
    throw invalid_argument("Unsupported Comparison Operator: " + op);
}

static shared_ptr<CondExprNode> applyBoolOperation(string op,
                                                   shared_ptr<CondExprNode> a,
                                                   shared_ptr<CondExprNode> b) {
  if (op == "&&")
    return make_shared<AndNode>(a, b);
  else if (op == "||")
    return make_shared<OrNode>(a, b);
  else if (op == "!")
    return make_shared<NotNode>(b);
  else
    throw invalid_argument("Unsupported Boolean Operator: " + op);
}
...
static string printStatement(shared_ptr<StatementNode> statement, int nesting) {
  ...
  if (assignStmt) {
    return to_string(assignStmt->getStatementNumber()) + indent +
           assignStmt->getVar()->getValue() + " = " +
           printExpr(assignStmt->getExpr()) + "\n";
  } else if (ifStmt) {
    string res = "";
    res += to_string(ifStmt->getStatementNumber()) + indent + "if (" +
           printCondExpr(ifStmt->getCondExpr()) + ") {\n";
    ...
    return to_string(callStmt->getStatementNumber()) + indent + "calls " +
           callStmt->getProc()->getName() + "\n" + callMapper(callStmt);
  } else if (readStmt) {
    return to_string(readStmt->getStatementNumber()) + indent +
           readStmt->getName() + " " + readStmt->getVar()->getValue() + "\n";
  } else if (printStmt) {
    return to_string(printStmt->getStatementNumber()) + indent +
           printStmt->getName() + " " + printStmt->getVar()->getValue() + "\n";
  } else if (whileStmt) {
    string res = "";
    res += to_string(whileStmt->getStatementNumber()) + indent + "while (" +
           printCondExpr(whileStmt->getCondExpr()) + ") {\n";
    std::vector<std::shared_ptr<StatementNode>> loopStmts =
        whileStmt->getStatements();
