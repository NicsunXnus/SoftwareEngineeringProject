...
shared_ptr<ProgramNode> ASTBuilder::parseProgram(shared_ptr<TokenizedProgram> tokenizedProgram) {
...
shared_ptr<StatementNode> ASTBuilder::parseStatement(shared_ptr<TokenizedStmt> statement) {
    ...
    if (auto tokenizedSemicolonStmt = dynamic_pointer_cast<TokenizedSemicolonStmt>(statement)) {
        ...
        if (statementContents[secondIndex]->getName() == "=") {
            ...
            if (containsProcedure(procedureName, ASTBuilder::procedureMap)) {
                callNode = make_shared<CallNode>(statementNumber, ASTBuilder::procedureMap[procedureName]);
                return callNode;
            }
            else {
                ...
                ASTBuilder::callMap.insert({ procedureName, callNode }); //store pointer to callNode, will use pointer to update the callNode after all procedures have been parsed
                return callNode;
            }
        }
...
static void applyExprNodeStackOperation(stack<shared_ptr<ExprNode>>& values, stack<string>& ops) {
...
shared_ptr<ExprNode> ASTBuilder::parseExpr(vector<shared_ptr<Token>> listOfTokens, int statementNumber) {
...
static void applyCondExprNodeStackOperation(stack<shared_ptr<CondExprNode>>& condValues, stack<shared_ptr<ExprNode>>& relValues, stack<string>& ops) {
...
shared_ptr<CondExprNode> ASTBuilder::parseCondExpr(vector<shared_ptr<Token>> listOfTokens, int statementNumber) {
