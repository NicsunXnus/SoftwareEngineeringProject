...
class TokenFactory {
 private:
  ...
  static std::shared_ptr<Token> generateCommonToken(
  ...
  static std::shared_ptr<Token> generateSimpleToken(
      std::string_view tokenName) {
    if (tokenName == "="sv) {
  ...
  // Generates a Identifier Token. REMINDER that this does not validate the
  // argument to ensure that it is a valid name
  static std::shared_ptr<Token> generateIdentifier(std::string_view tokenName) {
    return std::make_shared<IdentifierToken>(tokenName);
  }

  // Generates an Integer Literal Token. REMINDER that this does not validate
  // the argument to ensure that it is a valid number
 ...
 public:
  ...
  /// </summary>
  ///
  /// <param name="tokenName">The string to be passed to be turned into a
  /// token</param> <param name="forSimple">Whether the string is coming from a
  /// SIMPLE program or not</param> <param name="forceIdentifier">Whether to
  /// force the token to be an IdentifierToken</param> <returns>a shared pointer
  /// to the generated token</returns>
  static std::shared_ptr<Token> generateTokenForSimple(
      std::string tokenName, bool forceIdentifier = false) {
    if (tokenName.empty()) {
      std::cerr << ExceptionMessages::invalidToken << std::endl;
      return nullptr;
    }
  ...
  static std::shared_ptr<Token> generateTokenForSimple(
      std::string_view tokenName, bool forceIdentifier = false) {
