...
class Token {
 ...
 public:
  Token(std::string_view tokenName) : tokenName{tokenName} {};
  // Getter for the token name since the field is private.
  virtual std::string getName() { return this->tokenName; }
  // Overriden
  ...
  // Checks if the two tokens supplied are the same runtime type and same stored
  // tokenName. The arguments supplied MUST be pointers.
  ...
  static bool checkEquality(std::shared_ptr<Token> left,
                            std::shared_ptr<Token> right) {
    return checkEquality(left.get(), right.get());
  }

  // Overloaded method to take in shared_ptr on the left instead of regular
  // pointers
  static bool checkEquality(std::shared_ptr<Token> left, Token* right) {
    return checkEquality(left.get(), right);
  }

  // Overloaded method to take in shared_ptr on the right instead of regular
  // pointers
