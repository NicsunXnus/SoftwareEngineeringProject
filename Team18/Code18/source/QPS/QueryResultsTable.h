...
class QueryResultsTable {
 public:
  ...
  // Each column is reprsented by a map, key being header and value being its
  // contents It is assumed that there are no duplicate headers in the table. A
  // table is always sorted by headers.

  // Tested using: https://www.onlinegdb.com/online_c++_compiler#

  QueryResultsTable(vector<map<string, vector<string>>> _columns)
      : columns(_columns),
        isSignificant(getNumberOfCols() > 0 && getNumberOfRows() > 0) {
  ...
  int getNumberOfCols() { return columns.size(); }

  /**
   * Creates a new QueryResultsTable object that is the result of a cartesian
   * product between the row of elements of this table and the other table.
   *
   * @param other A shared pointer to a QueryResultsTable object that represents
   * the other tables.
  ...
  shared_ptr<QueryResultsTable> crossProduct(
    ...
    if (other->isEmpty() || this->isEmpty()) {
      if (other->getSignificant() && other->isEmpty() &&
          this->getSignificant() && !this->isEmpty()) {  // TRUE EMPTY X TABLE
        return make_shared<QueryResultsTable>(thisColumns);
      }
      if (this->getSignificant() && this->isEmpty() &&
          other->getSignificant() && !other->isEmpty()) {  // TABLE X TRUE EMPTY
        return other;
      }
    ...
    /*if you have 2 tables
       ...
       size(table2) etc; we have table1:              table2: s1, s2,   s3 s4,
       s5, s6 1  ,  2,   3           a, b,  c 4,  5,   6            d,  e,  f
   ...
   * intersection between this table and the other table.
   *
  ...
  shared_ptr<QueryResultsTable> innerJoin(shared_ptr<QueryResultsTable> other) {
    vector<map<string, vector<string>>> thisMap = this->columns,
                                        otherMap = other->columns, innerJoined;
    vector<string> thisHeaders = getHeaders();
    // Get the number of columns and rows in both tables
    int thisColNums = getNumberOfCols(), thisRowNums = getNumberOfRows(),
    ...
    for (int thisCol = 0; thisCol < thisColNums; thisCol++) {
      ...
      // at the same time, check for similar headers and record down the pairs
      // of column numbers from both tables
      for (int otherCol = 0; otherCol < otherColNums; otherCol++) {
   ...
   * @param other A shared pointer to a QueryResultsTable object representing
   * the other table.
   * this table's column values is a subset of the other table's column values.
   * @return A shared pointer to a newly created QueryResultsTable object
   * (single column)
   */
  ...
  /**
   * A static method that creates an empty QueryResultsTable with a single
   * header.
   *
   * @param header The header of the column, represented as a string.
   */
  static shared_ptr<QueryResultsTable> createEmptyTableWithHeader(
      string header) {
  ...
  /**
   * A static method that creates a new QueryResultsTable object with a single
   * column.
   *
   * @param header The header of the column, represented as a string.
   * @param columnValues A vector of strings representing the values in the
   * column. The vector should have the same length for all columns in the
   * table.
   * @return A shared_ptr to the newly created QueryResultsTable object.
   */
  ...
  /**
   * A static method that flattens a map into a table of two columns, left
   * column representing the key and right column representing the values
   *
   * @param headers A vector of strings representing the headers of the table to
   * be created.
   * @param columnValues A map of a string to a vector of strings representing
   * the unflattened map.
   ...
   *
   * @param header The header of the column, represented as a string.
   * @param columnValues An unordered set of strings representing the values in
   * the column. The vector should have the same length for all columns in the
   * table.
   * @return A shared_ptr to the newly created QueryResultsTable object.
   */
  static shared_ptr<QueryResultsTable> createTable(
  ...
  /**
   * A static method that creates a new QueryResultsTable object with the
   * provided headers and column values.
   *
   * @param headers A vector of strings representing the headers of the table to
   * be created.
   * @param columnValues A vector of a vector of strings representing the column
   * values.
   * @return A shared pointer to the newly created QueryResultsTable object.
   */
  static shared_ptr<QueryResultsTable> createTable(
   ...
   *
   * @param deleteHeader The header of the column to be deleted, represented as
   * a string.
   */
  void deleteColumn(string deleteHeader) {
    ...
    if (it != headers.end()) {
      int index = distance(headers.begin(), it);
      bool isSignificant =
          getNumberOfRows() > 0;  // table will be "empty" after drop
  ...
  /**
   * Renames a column indicated by the header given with the new header
   * provided.
   *
   ...
   * @param targets A vector of strings representing the values the column
   * should only have.
   * @return A shared pointer to the newly created QueryResultsTable object
   * representing the old table but only containing rows having only values of
   * the "targets" of the column with header "key"
   */
  shared_ptr<QueryResultsTable> filter(
      string key,
      vector<string>
          targets) {  // assuming there is no spacing/wrong capitalisation in
                      // key & there is no duplicates in targets
    if (isEmpty()) {
    ...
    vector<map<string, vector<string>>> filteredTableColumns =
        this->createColumnsWithHeaders(headers);
          ...
          if (targetColumn[row] == target) {
            for (int col = 0; col < headers.size(); col++) {
              filteredTableColumns[col].begin()->second.emplace_back(
                  this->columns[col].begin()->second[row]);
            }
          }
        }
      }
   ...
   * should only have.
   * @return A shared pointer to the newly created QueryResultsTable object
   * representing the old table but only containing rows having only values
   * where col1 = col2
   */
   ...
   * @param other A shared pointer to a QueryResultsTable object representing
   * the other table
   * @return A boolean value, with a true value representing same headers for
   * both, and a false value for otherwise.
   */
  ...
  vector<map<string, vector<string>>> getColumns() { return this->columns; }
  ...
  void setSignificant(bool empty) { isSignificant = empty; }

  void flipSignificant() { isSignificant = !isSignificant; }

  bool getSignificant() { return isSignificant; }

  bool isEmpty() { return getNumberOfCols() == 0 || getNumberOfRows() == 0; }
 ...
 private:
  ...
  // A significant table represents the boolean "true", regardless if the table
  // is empty or not This is especially important for queries that involve
  // booleans, such as with clause: "this" = "this"
  string primaryKey = "";  // primary key of the table, especially important for
                           // selecting attributes
  // e.g. Select constant.value, primary key will be constant.value, other key
  // will be constant
  ...
  vector<map<string, vector<string>>> createColumnsWithHeaders(
