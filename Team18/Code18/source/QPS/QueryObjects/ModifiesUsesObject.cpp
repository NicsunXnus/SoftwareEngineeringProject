...
shared_ptr<QueryResultsTable> UsesObject::callAndProcess(
    shared_ptr<DataAccessLayer> dataAccessLayer) {
  return handleUses(dataAccessLayer, USES);
}

shared_ptr<QueryResultsTable> UsesEntityObject::callAndProcess(
    shared_ptr<DataAccessLayer> dataAccessLayer) {
  return handleUses(dataAccessLayer, USES);
}

shared_ptr<QueryResultsTable> ModifiesObject::callAndProcess(
    shared_ptr<DataAccessLayer> dataAccessLayer) {
  return handleModifies(dataAccessLayer, MODIFIES);
}

shared_ptr<QueryResultsTable> ModifiesEntityObject::callAndProcess(
    shared_ptr<DataAccessLayer> dataAccessLayer) {
  return handleModifies(dataAccessLayer, MODIFIES);
}